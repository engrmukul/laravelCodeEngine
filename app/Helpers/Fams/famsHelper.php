...
//rename from fixedAssetHelper.php
...
function maintenanceDelegationProcess($fam_maint_requests_id = array(), $delegation_type, $comments = '', $additional_data = '')
{
  if (is_array($fam_maint_requests_id) && !empty($fam_maint_requests_id)) {
    $req = DB::table('fam_maint_requests')
            ->selectRaw('GROUP_CONCAT(fam_requests_no) as fam_requests_no')
            ->whereIn('fam_maint_requests_id', $fam_maint_requests_id)
            ->first();
    $code = explode(',', $req->fam_requests_no);
    //debug($code, 1);
