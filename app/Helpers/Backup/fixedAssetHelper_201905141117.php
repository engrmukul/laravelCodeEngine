...
function generateAssetTypeFormFields($products_id, $is_multiple = false, $multiple_no = 1, $cols = 2)
{
  if ($is_multiple == 'true') {
    ...
    generateAssetSpecification($products_id, $rows, 'ntr');
    
    echo '<div class="section-divider"></div>';
    
    // Then trackable
    $rows = getProductSpecStandards($products_id, $is_multiple, 1);
    generateAssetSpecification($products_id, $rows, 'tra', $multiple_no);
  } else {
    // Both trackable and not trackable
    $rows = getProductSpecStandards($products_id, $is_multiple);
    generateAssetSpecification($products_id, $rows, 'tnt');
  }
}
...
function generateAssetSpecification($products_id, $rows, $index, $multiple_no = 1, $cols = 2)
{
  if ($rows->count() > 0) {
    $col = (12 / $cols);
    ?>
...
<div class="panel panel-default">
  <div class="panel-body">
    <div class="row">
      ...
      $i = 0;
      foreach ($rows as $row) {
        $analysis_value = getAnalysisValue($products_id, $row->product_spec_standards_name);
        if ($i == $cols) {
          $i = 0;
          ?>
        </div>
...
function getAnalysisValue($products_id, $product_spec_standards_name)
{
  $analysis_value = DB::table('fam_item_spec_details')
                ->join('fam_items', 'fam_item_spec_details.fam_items_id', '=', 'fam_items.fam_items_id')
                ->where(['products_id' => $products_id, 'standards_name' => $product_spec_standards_name])
